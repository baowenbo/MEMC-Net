...
def VBM4D_loader(root, im_path, video_name, task, task_param,
                     # DownResizer=None,UpResizer=None,
                     input_frame_size = ( 128,128,3)):

    if task == 'denoise':
        root_input = os.path.join(root, 'noisy_sigma' + str(int(task_param[0])) + '_imgs_3ch', video_name)
    elif task == 'deblock':
        root_input = os.path.join(root, 'blocky_H264qp' + str(int(task_param[0])) + '_imgs', video_name)

    root_target =os.path.join(root,'noise_free_imgs',video_name)
    ...
    im  = []
    target = []
    cur_idex = int(im_path[-8:-4])
    for i in range(0,7):
        temp = os.path.join(root_input, im_path[:-8]+
                            str((cur_idex-3+i)).zfill(4)+im_path[-4:])
        path.append(temp)
...
class ListDataset(data.Dataset):
    def __init__(self, root, path_list,videos_list,
                 task = 'sr', task_param = [20],  loader=VBM4D_loader): #transform=None, target_transform=None, co_transform=None,
