...
arguments_strFirst = './images/frame_0001.png'#fframe10.png'#irst.png'
arguments_strSecond = './images/frame_0002.png' #sframe11.png' #econd.png'
	...
	def forward(self, variableFirst, variableSecond):
		...
		for intLevel in range(len(variableFirst)):
			...
			if variableUpsampled.size(2) != variableFirst[intLevel].size(2):
				variableUpsampled = torch.nn.functional.pad(variableUpsampled, [0, 0, 0, 1], 'replicate')
			if variableUpsampled.size(3) != variableFirst[intLevel].size(3):
				variableUpsampled = torch.nn.functional.pad(variableUpsampled, [0, 1, 0, 0], 'replicate')
...
def estimate(tensorInputFirst, tensorInputSecond):
	...
	if True:
		...
		for kk in  range(0,10):
			...
			temp = 	moduleNetwork(variableInputFirst, variableInputSecond)
...
if __name__ == '__main__':
	tensorInputFirst = torch.FloatTensor(numpy.rollaxis(numpy.asarray(PIL.Image.open(arguments_strFirst))[:384,:1024,:], 2, 0).astype(numpy.float32) / 255.0)
	tensorInputSecond = torch.FloatTensor(numpy.rollaxis(numpy.asarray(PIL.Image.open(arguments_strSecond))[:384,:1024,:], 2, 0).astype(numpy.float32) / 255.0)
